...
export function JobPasteForm() {
  ...
  function runProcess(force: boolean) {
    setMessage(null);
    setFormError(null);
    setShowDetail(false);
    ...
    startTransition(async () => {
      ...
      setMessage(
        `Vacante ${verbo}: ${result.cargo} en ${result.empresa} (${result.habilidades.length} habilidades: ${requeridas} requeridas, ${valoradas} valoradas, ${blandas} blandas).`,
      );
    });
  }
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
      <div className="flex flex-col gap-2">
        ...
        <Input
          ...
          className={cn(
            "bg-white/70",
            fieldErrors.url && "border-destructive focus-visible:ring-destructive/30",
          )}
          disabled={isPending}
        ...
        />
        {fieldErrors.url ? (
          <p id="url-oferta-error" className="text-xs text-destructive" role="alert">
            {fieldErrors.url}
          </p>
        ) : (
          <p className="text-xs text-muted-foreground">
            Obligatoria para seguimiento. LinkedIn acepta tanto{" "}
            <code className="text-[0.7rem]">/jobs/view/…</code> como búsqueda con{" "}
            <code className="text-[0.7rem]">currentJobId</code> (se unifican).
            También valen elempleo, infojobs, indeed u otros.
          </p>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        />
        {fieldErrors.texto ? (
          <p id="vacante-error" className="text-xs text-destructive" role="alert">
            {fieldErrors.texto}
          </p>
        ) : null}
      </div>
      ...
      {duplicate ? (
        ...
        >
          ...
          <p className="mt-1 text-amber-900/90">
            ...
            Puedes descartar el envío o agregar de todos modos para actualizar el
            texto y sumar solo habilidades nuevas.
          </p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          {isPending && !duplicate ? "Analizando…" : "Extraer y guardar"}
        </Button>
      </div>
    </form>
  );
